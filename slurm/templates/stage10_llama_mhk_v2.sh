...
#SBATCH --job-name=physmon_s10_llm_mhk_v2
...
# Candidate A: top contribution heads at the best attention-analysis layer (L19).
python -u scripts/run_causal_patching.py \
  ...
  --patch-layers 17 18 19 20 21 \
  --patch-site resid_post_last_prompt \
  --patch-mode head_knockout \
  --multi-head-knockout 0 12 14 26 \
  --behavioural-jsonl results/stage6/behavioural_full_rerun/llama_primary_20260616T085448Z.jsonl \
  --output-dir results/stage10/llama_mhk_v2/attn_layer19/ \
  --stage 10 \
  --seed 42

# Candidate B: top individual causal heads from the clean 19/20/21 head-selection sweep.
python -u scripts/run_causal_patching.py \
  --model-role llama_primary \
  --family-dir results/stage6/generated_full_benchmark/ \
  --patch-families "${FAMILIES[@]}" \
  ...
  --patch-site resid_post_last_prompt \
  --patch-mode head_knockout \
  --multi-head-knockout 12 0 23 5 \
  --behavioural-jsonl results/stage6/behavioural_full_rerun/llama_primary_20260616T085448Z.jsonl \
  --output-dir results/stage10/llama_mhk_v2/causal_sweep/ \
  --stage 10 \
  --seed 42
