#!/bin/bash
# Stage 8 variance ensemble probe on Qwen across layers 15-20 (CPU-only).

#SBATCH --job-name=physmon_s8_qwen_ens
...
echo "=== PhysMon Stage 8 Probe Start (Qwen variance ensemble) ==="
...
python -u scripts/run_probing.py \
  ...
  --output-dir results/stage6/probing/variance_ensemble/ \
  --stage 8 \
  --seed 42

echo "=== PhysMon Stage 8 Probe Complete (Qwen variance ensemble) ==="
