#!/bin/bash
# Stage 8 DeepSeek-R1 smoke behavioural run on 3 families.

#SBATCH --job-name=physmon_stage8_deepseek_smoke
...
echo "=== PhysMon Stage 8 DeepSeek Smoke Start ==="
...
python -u scripts/run_behavioural.py \
  ...
  --output-dir results/stage8/behavioural_deepseek_smoke/ \
  --device cuda \
  --device-map auto \
  --family-filter CM_B_UM_059 EL_B_002 CM_C_005 \
  --max-new-tokens 512 \
  --seed 42

echo "=== PhysMon Stage 8 DeepSeek Smoke Complete ==="
