#!/bin/bash
# Stage 6 Phase D2 full-benchmark behavioural sweep on Llama.

#SBATCH --job-name=physmon_stage6_d2r_llama
#SBATCH --partition=gpu_a100_8
#SBATCH --gres=gpu:nvidia_a100-sxm4-80gb:1
#SBATCH --cpus-per-task=8
#SBATCH --mem-per-gpu=90G
#SBATCH --time=10:00:00
#SBATCH --output=/scratch/pabitra/physmon/slurm_logs/stage6_d2r_llama_%j.out
#SBATCH --error=/scratch/pabitra/physmon/slurm_logs/stage6_d2r_llama_%j.err
...
cd "$PHYSMON_ROOT"
mkdir -p results/stage6/behavioural_full_rerun
echo "pwd=$(pwd)"
which python
python --version
echo "starting_run_behavioural_llama_stage6_d2_rerun"

python -u scripts/run_behavioural.py \
  --model-role llama_primary \
  --family-dir results/stage6/generated_full_benchmark \
  --output-dir results/stage6/behavioural_full_rerun \
  --device cuda \
  --max-new-tokens 128 \
  --seed 42
