#!/bin/bash
# Stage 8 multi-head knockout over the identified layer-18 head set on H100.

#SBATCH --job-name=physmon_stage8_mhk_h100
...
echo "=== PhysMon Stage 8 Multi-Head Knockout Start (H100) ==="
...
python -u scripts/run_causal_patching.py \
  ...
  --output-dir results/stage8/multi_head_knockout/ \
  --stage 8 \
  --seed 42

echo "=== PhysMon Stage 8 Multi-Head Knockout Complete (H100) ==="
