#!/bin/bash
# Stage 8 causal activation patching on Qwen (GPU, highest priority).

#SBATCH --job-name=physmon_stage8_patch_qwen
...
echo "=== PhysMon Stage 8 Causal Patching Start (Qwen) ==="
...
python -u scripts/run_causal_patching.py \
  ...
  --output-dir results/stage8/causal_patching/ \
  --stage 8 \
  --seed 42

echo "=== PhysMon Stage 8 Causal Patching Complete (Qwen) ==="
