#!/bin/bash
# Stage 8 variance ensemble probe on Qwen without Cue C families (CPU-only).

#SBATCH --job-name=physmon_s8_qwen_ens_nc
...
echo "=== PhysMon Stage 8 Probe Start (Qwen variance ensemble, no Cue C) ==="
...
python -u scripts/run_probing.py \
  ...
  --probe-type variance_ensemble \
  --ensemble-layers 15 16 17 18 19 20 \
  --pca-per-layer 50 \
  ...
  --output-dir results/stage6/probing/variance_ensemble_no_cue_c/ \
  --stage 8 \
  --seed 42

echo "=== PhysMon Stage 8 Probe Complete (Qwen variance ensemble, no Cue C) ==="
