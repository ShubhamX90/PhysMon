#!/bin/bash
# Stage 8 cue-token causal activation patching on Qwen.

#SBATCH --job-name=physmon_stage8_patch_cue_token
...
echo "=== PhysMon Stage 8 Cue-Token Causal Patching Start (Qwen) ==="
...
python -u scripts/run_causal_patching.py \
  ...
  --patch-site resid_post_cue_token \
  --behavioural-jsonl results/stage6/behavioural_full_rerun/qwen_primary_20260616T091228Z.jsonl \
  --output-dir results/stage8/causal_patching_cue_token/ \
  --stage 8 \
  --seed 42

echo "=== PhysMon Stage 8 Cue-Token Causal Patching Complete (Qwen) ==="
